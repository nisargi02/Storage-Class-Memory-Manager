...
static struct node *delete_node(struct avl *avl, struct node *root, const char *item, int *success)
{
    int cmp;                   
    int balance_factor;
    struct node *temp;
    ...
    if (cmp < 0) {
        root->left = delete_node(avl, root->left, item, success);
    } else if (cmp > 0) {
        root->right = delete_node(avl, root->right, item, success);
    } else {
        
        *success = 1;
        avl->state->items -= root->count;
        avl->state->unique--;
        ...
        if ((root->left == NULL) || (root->right == NULL)) {
            ...
            root->right = delete_node(avl, root->right, temp->item, success);
        }
    }
...
int avl_remove(struct avl *avl, const char *item)
{
    ...
    avl->state->root = delete_node(avl, avl->state->root, item, &success);
